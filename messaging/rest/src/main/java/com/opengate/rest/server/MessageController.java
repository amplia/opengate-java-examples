...
public class MessageController {
    ...
    public ModelAndView handleMessage(@PathVariable String messageType, @RequestBody Ogmessage ogmessage) {


        System.out.println("Ogmessage received: " + ogmessage + " with type: " + messageType);
        ...
        for (Object object : list) {
            try {
                ...
                Method method = clazz.getMethod("getValue", (Class<?>) null);
                boolean isBoolDataUnit= method == null;
                if (isBoolDataUnit) {
                    method = clazz.getMethod("isValue", (Class<?>) null);
                }
                System.out.println("\t\t" + method.invoke(object, (Object) null));
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
        if (messageType.equals("requests")) {
            System.out.println("Request received. Sending response with id: " + ogmessage.getId());
            ...
            ResponseEntity<MessageStatus> response = restTemplate.exchange(responseUrl, HttpMethod.POST,
